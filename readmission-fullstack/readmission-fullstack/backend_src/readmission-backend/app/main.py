...
app = FastAPI(title="Hospital Readmission XAI API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def load_dataset(path: str) -> pd.DataFrame:
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Dataset zip not found at {path}. Put the zip next to the backend folder or set DATASET_ZIP."
        )
...
def explain_prediction(input_df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    contributions = row * model.named_steps["classifier"].coef_[0]
    df_exp = pd.DataFrame({
